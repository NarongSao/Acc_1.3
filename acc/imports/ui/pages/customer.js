...
// Declare template
let indexTmpl = Template.Acc_customer,
    actionTmpl = Template.Acc_customerAction,
    newTmpl = Template.Acc_customerNew,
    editTmpl = Template.Acc_customerEdit,
    showTmpl = Template.Acc_customerShow;
...
indexTmpl.onCreated(function () {
    ...
    this.filter = new ReactiveTable.Filter('acc.customerByBranchFilter', ['branchId']);
    this.autorun(()=> {
...
indexTmpl.helpers({
    ...
    tableSettings(){
        let i18nPrefix = 'acc.customer.schema';

        reactiveTableSettings.collection = 'acc.reactiveTable.customer';
...
editTmpl.onCreated(function () {
    this.autorun(()=> {
        this.subscribe('acc.customer', {_id: this.data._id});
    });
});
...
showTmpl.helpers({
    i18nLabel(label){
        let i18nLabel = `acc.customer.schema.${label}.label`;
        return i18nLabel;
    },
    data () {
        let data = Customer.findOne(this._id);
